# frozen_string_literal: true

# Registration model representing a user's registration to a session.
#
# Handles:
# - Registration validations (level, credits, schedule conflicts, session full)
# - Weekly training limit (one training per week, except current week)
# - Waitlist management
# - Deadline bypass for admins/coaches
# - Private coaching registration control
class Registration < ApplicationRecord
  belongs_to :user
  belongs_to :session

  enum :status, { confirmed: 0, waitlisted: 1 }

  # Allows creating registrations for private coachings even though public registrations are closed
  attr_accessor :allow_private_coaching_registration

  # Allows creating registrations after the registration deadline (17h) for admins and session coaches
  attr_accessor :allow_deadline_bypass

  after_initialize do
    self.status ||= :confirmed if has_attribute?(:status)
  end

  validates :user_id, uniqueness: { scope: :session_id }

  validate :enough_credits?
  validate :can_register?
  validate :no_schedule_conflict
  validate :weekly_training_limit

  def can_register?
    validate_registration_opening
    validate_private_coaching_access
    validate_level
    validate_session_full
    validate_credits
  end

  def can_register_with_reason
    return validate_opening_with_reason unless validate_opening_with_reason[0]
    return [ false, "Les coachings privés ne sont pas ouverts à l'inscription." ] if private_coaching_closed?
    return [ false, "Ce n'est pas ton niveau d'entrainement." ] unless level_allowed?
    return [ false, "Session complète." ] if confirmed? && session.full?
    return [ false, "Tu es déjà inscrit à une autre session sur le même créneau." ] if confirmed? && schedule_conflict_exists?

    return [ false, "Tu n'as pas assez de crédits." ] unless enough_credits?

    [ true, nil ]
  end

  def enough_credits?
    user.balance.amount >= required_credits_for(user)
  end

  # Waitlisted users have not paid yet; do not require/refund credits
  def required_credits_for(user)
    return 0 if session.coaching_prive? || waitlisted?
    session.price.to_i
  end

  def level_allowed?
    return true unless session.entrainement?
    return true if session.levels.empty?

    user_level_ids = collect_user_level_ids
    return false if user_level_ids.empty?

    (user_level_ids & session.level_ids).any?
  end

  def no_schedule_conflict
    return if waitlisted?
    return unless schedule_conflict_exists?

    errors.add(:session, "chevauche une autre session à laquelle tu es inscrit")
    errors.add(:base, "Tu es déjà inscrit à une autre session sur le même créneau.")
  end

  # Enforce: at most one training (entrainement) per week, except for the current week
  def weekly_training_limit
    return if waitlisted?
    return unless confirmed?
    return unless session&.entrainement?
    return if current_week?

    return unless already_registered_this_week?

    errors.add(:base, "Tu as déjà un entraînement sur cette semaine. Une seule inscription est autorisée (hors semaine en cours).")
  end

  private

  def validate_registration_opening
    open_ok, open_reason = session.registration_open_state_for(user, skip_deadline: allow_deadline_bypass)
    errors.add(:base, open_reason) unless open_ok
  end

  def validate_private_coaching_access
    return unless session.coaching_prive? && !allow_private_coaching_registration

    errors.add(:base, "Les coachings privés ne sont pas ouverts à l'inscription.")
  end

  def validate_level
    return if level_allowed?

    errors.add(:user, "n'a pas le bon niveau pour cet entraînement")
    errors.add(:base, "Ce n'est pas ton niveau d'entrainement.")
  end

  def validate_session_full
    return unless confirmed? && session.full?

    errors.add(:status, "impossible: session complète")
    errors.add(:base, "Session complète.")
  end

  def validate_credits
    return if enough_credits?

    errors.add(:user, "n'a pas assez de crédits")
    errors.add(:base, "Pas assez de crédits.")
  end

  def validate_opening_with_reason
    session.registration_open_state_for(user)
  end

  def private_coaching_closed?
    session.coaching_prive? && !allow_private_coaching_registration
  end

  def schedule_conflict_exists?
    user
      .sessions_registered
      .where("start_at < ? AND end_at > ?", session.end_at, session.start_at)
      .where.not(id: session.id)
      .exists?
  end

  def collect_user_level_ids
    user_level_ids = user.respond_to?(:levels) ? user.levels.pluck(:id) : []

    if user_level_ids.empty? && user.respond_to?(:level_id)
      user_level_ids = [ user.level_id ].compact
    end

    user_level_ids
  end

  def current_week?
    today_week_start = Time.zone.today.beginning_of_week(:monday)
    session_week_start = session.start_at.in_time_zone.beginning_of_week(:monday).to_date
    session_week_start == today_week_start
  rescue StandardError
    false
  end

  def already_registered_this_week?
    week_start = session.start_at.in_time_zone.beginning_of_week(:monday)
    week_end = session.start_at.in_time_zone.end_of_week(:monday)

    user
      .sessions_registered
      .where(session_type: "entrainement")
      .where(start_at: week_start..week_end)
      .where.not(id: session.id)
      .exists?
  end
end
